...
def get_gen_model(point_cloud, is_training, scope, bradius = 1.0, reuse=None,use_bn = False,use_ibn = False,
    ...
    with tf.variable_scope(scope,reuse=reuse) as sc:
        ...
        with tf.variable_scope('up_layer', reuse=reuse):
            upsampled_feature_list = []
            concat_feat = tf.concat([concat_feat,tf.expand_dims(tf.expand_dims(dist,axis=-1),axis=-1)],axis=-1)
            for i in range(up_ratio):
                upsampled_feature = tf_util2.conv2d(concat_feat, 256, [1, 1],
                                              padding='VALID', stride=[1, 1],
                                              bn=False, is_training=is_training,
                                              scope='conv1_%d' % (i), bn_decay=bn_decay)

                upsampled_feature = tf_util2.conv2d(upsampled_feature, 128, [1, 1],
                                             padding='VALID', stride=[1, 1],
                                             bn=use_bn, is_training=is_training,
                                             scope='conv2_%d' % (i),
                                             bn_decay=bn_decay)
                upsampled_feature_list.append(upsampled_feature)
                upsampled_feature = tf.concat(upsampled_feature_list, axis=1)

        r_coord = tf_util2.conv2d(upsampled_feature, 64, [1, 1],
                                padding='VALID', stride=[1, 1],
                                bn=False, is_training=is_training,
        ...
        if False:
            ...
            edge_dist, idx1 = tf.nn.top_k(-tf.tile(dist,(1,up_ratio)), num_addpoint)
        ...
        edge_dist = tf.gather_nd(tf.tile(dist,(1,up_ratio)),idx)
        # edge_dist = -edge_dist
