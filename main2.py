...
parser.add_argument('--log_dir', default='../model/NEWCAD_generator1_1k_updist_lesspoint', help='Log dir [default: log]')
...
class Network(object):
    ...
    def build_graph(self,is_training=True,scope='generator'):
        ...
        self.edge_mask = tf.to_float(tf.less_equal(self.pred_edgedist, 0.1))
        ## The following code is okay when the batch size is 1
        indics = tf.where(tf.less_equal(self.pred_edgedist,0.1)) #(?,2)
        ...
        self.pointclouds_dist_truncated = tf.minimum(0.7,self.pointclouds_dist)
        self.pred_dist = tf.minimum(0.7,tf.maximum(0.0,self.pred_dist))

        self.dist_mseloss = (self.pointclouds_dist_truncated - self.pred_dist) ** 2
        ...
        self.perulsionloss = 10*model_utils.get_uniform_loss1(self.pred_coord)
        ...
        pointclouds_input_summary = tf.summary.image('pointcloud_input', self.pointclouds_image_input, max_outputs=1)
        self.pointclouds_image_pred = tf.placeholder(tf.float32, shape=[None, 500, 1500, 1])
        pointclouds_pred_summary = tf.summary.image('pointcloud_pred', self.pointclouds_image_pred, max_outputs=1)
        self.pointclouds_image_gt = tf.placeholder(tf.float32, shape=[None, 500, 1500, 1])
        pointclouds_gt_summary = tf.summary.image('pointcloud_gt', self.pointclouds_image_gt, max_outputs=1)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx in range(self.fetchworker.num_batches):
            ...
            _, summary, step, pred_coord, pred_edgecoord, edgemask, gen_loss_emd = self.sess.run(
                [self.pre_gen_train,self.merged, self.step, self.pred_coord, self.pred_edgecoord, self.edge_mask, self.edge_loss], feed_dict=feed_dict)
    ...
    def test_hierarical_prediction(self):
        data_folder = '../../PointSR_data/CAD4/simu_noise_free'
        ...
        with tf.Session(config=config) as sess:
            ...
            for point_path in samples:
                ...
                for seed in tqdm(seeds):
                    ...
                    up_edgepoint_list.append(up_edgepoint)
                ...
                data_provider.save_xyz(path, input)

                pred = np.concatenate(up_point_list,axis=0)
                path = os.path.join(save_path, point_path.split('/')[-1][:-4] + ".xyz")
                data_provider.save_xyz(path, pred)

                pred_edge = np.concatenate(up_edgepoint_list,axis=0)
                ...
                path = os.path.join(save_path, point_path.split('/')[-1][:-4] + "_edge.ply")
                ...
                path = os.path.join(save_path, point_path.split('/')[-1][:-4] + "_predictedge.ply")
