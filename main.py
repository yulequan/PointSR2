...
class Network(object):
    ...
    def build_graph(self,is_training=True,scope='generator'):
        ...
        self.edge_mask = tf.to_float(tf.less_equal(self.pred_edgedist, 0.15))
        ## The following code is okay when the batch size is 1
        indics = tf.where(tf.less_equal(self.pred_edgedist,0.15)) #(?,2)
    ...
    def patch_prediction(self, patch_point,idx,sess):
        ...
        with tf.Session(config=config) as sess:
            ...
            for point_path in samples:
                ...
                for seed in tqdm(seeds):
                    ...
                    up_point,up_edgepoint,up_edgedist = self.patch_prediction(point, idx,sess)
                    inputs.append(point)
